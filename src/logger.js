    ...
    try {
      if (typeof window !== 'undefined') {
        window.addEventListener('error', (ev) => {
          this.error(`window.error: ${ev.message} at ${ev.filename}:${ev.lineno}:${ev.colno}`)
        })
        window.addEventListener('unhandledrejection', (ev) => {
          this.error(`unhandledrejection: ${ev.reason && ev.reason.stack ? ev.reason.stack : String(ev.reason)}`)
        })
      }
    } catch (e) {
      // ignore
